(function () {
  ...
  function updateTargetSelection(ids) { state.targetIds = normalizeTargetSelection(ids); setSelectedTargets(state.targetIds); renderTargetChips(byId("gqo-target-label"), state.targetIds); render(); }
  function openCurrentSelection(panel) { openPanel(panel, normalizeTargetSelection(selectedTargets().length ? selectedTargets() : state.targetIds)); byId("gqo-target-menu").hidden = true; byId("gqo-target-button").setAttribute("aria-expanded", "false"); }
  function openPanel(panel, ids, item) {
    state.targetIds = normalizeTargetSelection(targetIds(item || { targetIds: Array.isArray(ids) ? ids : [ids] })); state.editingId = item?.id || "";
  ...
  function savePanel(panel) {
    const ids = normalizeTargetSelection(state.targetIds.length ? state.targetIds : selectedTargets()); const comment = byId("gqo-comment").value.trim(); if (!comment) return;
  ...
  function wire(panel) {
    ...
    byId("gqo-comment-current").addEventListener("click", () => openCurrentSelection(panel)); byId("gqo-refresh").addEventListener("click", () => { refreshTargetOptions(); refreshWorkspace(); }); byId("gqo-list-toggle").addEventListener("click", () => setCollapsed(!state.collapsed)); byId("gqo-cancel").addEventListener("click", () => { state.editingId = ""; panel.hidden = true; render(); }); byId("gqo-save").addEventListener("click", () => savePanel(panel));
  }
